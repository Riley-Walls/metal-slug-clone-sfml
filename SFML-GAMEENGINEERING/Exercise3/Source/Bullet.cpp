...
Bullet::Bullet(b2World& mB2World, const TextureHolder& textures, float x, float y)
	: rectSourceSprite(0, 0, 39, 15)
	, mSprite(textures.get(Textures::Bullet), rectSourceSprite)
	, mB2Body(nullptr)
	, mVelocity(0, 0)
	, mB2World(mB2World)
{
	...
	*/
	...
	CubeFixtureDef.filter.categoryBits = World::B2CollisionBits::BULLET_BIT;
...
void Bullet::SetVelocity(int x, int y) {
	mVelocity.x = x;
	mVelocity.y = y;
}
...
void Bullet::SetTarget(Hero* hero)
{
	mTargetHero = hero;
}
...
void Bullet::attackCurrentTarget()
{
